...
impl PrimeSuccessorPredicate {
    ...
    pub fn try_mutate(&mut self, current: u64, next: u64) -> Result<(), &'static str> {
        if self.state == SigState::Accepted {
            return Err(
                "State immutability invariant violated: Cannot mutate an accepted Prime Successor Predicate.",
            );
        }
...
impl SigCompiler {
    ...
    pub fn new(language: &Language) -> Result<Self, &'static str> {
        ...
        parser
            ...
            .map_err(|_| "Failed to set tree-sitter language for PIRTM")?;
        Ok(Self { parser })
    }
    ...
    pub fn parse_and_enforce(&mut self, source_code: &str) -> Result<Tree, String> {
        let tree = self
            ...
            .ok_or("Failed to parse PIRTM source")?;
...
mod verification {
    ...
    fn verify_immutability_invariant() {
        ...
        kani::assert(
            predicate.state == SigState::Accepted,
            "State must remain strictly Accepted",
        );
        kani::assert(
            predicate.current_prime == current,
            "Current prime metric must not drift",
        );
        kani::assert(
            predicate.next_prime == next,
            "Next prime metric must not drift",
        );
    }
}
