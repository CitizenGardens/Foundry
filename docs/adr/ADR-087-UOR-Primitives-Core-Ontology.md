# ADR-087: UOR Primitives Type Family as Core Ontology
...
@[proof]
theorem standard_instance_sound : Primitives := by
  exact Standard

end Core.UOR
```

### 2. Certificate Well-Formed
```lean
namespace Core.UOR

structure Certificate (P : Primitives) where
  method : Option P.String
...
@[proof]
theorem certificate_well_formed (cert : Certificate Standard) :
  cert.method.isSome ∨ cert.certifies.isSome := by
  cases cert.method with
  | some _ => exact Or.inl (by simp)
  | none => cases cert.certifies with
    | some _ => exact Or.inr (by simp)
    | none => simp
